              ...
              filteredProjects.map((project) => {
                ...
                return (
                  ...
                  >
                    ...
                    {project.thumbnail && (
                      <div className={`absolute inset-0 w-full h-full z-0 overflow-hidden transition-opacity duration-300 ${isHovered && (project.youtubeEmbed || project.videoUrl) ? "opacity-0" : "opacity-100"}`}>
                        <img
                    ...
                    {/* YouTube iframe — only mounts on hover, no branding */}
                    {isHovered && project.youtubeEmbed && (
                      <div className="absolute inset-0 w-full h-full pointer-events-none z-10 overflow-hidden">
                        <iframe
                          className={`w-full h-full pointer-events-none ${isShort ? "scale-100" : "scale-[1.35]"}`}
                    ...
                    {/* MP4 video — plays on hover */}
                    {project.videoUrl && (
                      <div className={`absolute inset-0 w-full h-full pointer-events-none z-10 overflow-hidden transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"}`}>
                        <video
                    ...
                    {/* Play icon shown on thumbnail before hover */}
                    {!isHovered && (project.youtubeEmbed || project.videoUrl) && (
                      <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                        <div className="w-12 h-12 rounded-full bg-black/60 border border-white/20 backdrop-blur-sm flex items-center justify-center">
